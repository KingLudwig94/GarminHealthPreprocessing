...
from garmin_health_processing.utils import fillNanIslands, find_consecutive, findNanIsland, moving_average, \
    polifit_nan, r2, retime, signal_zerocrossings, graph, showpreprocess
...
class HRV:
    ...
    def computeWindow(self, data: dict, windowLength: int, windowSeparation: int, sr) -> str:
        ...
        while (i * windowSeparation + n < len(items)):
            # print(i)
            res = DataFrame(self.compute(dict(zip(offsets[i * windowSeparation:i * windowSeparation + n],
                                                  items[i * windowSeparation: i * windowSeparation + n])),
                                         items[i * windowSeparation: i * windowSeparation + n]), index=[offsets[i]])
            out = concat([out, res])
            i += 1
        return out.to_csv()
    ...
    def compute(self, data: dict, dataval: np.array) -> dict:
        ...
        out.update({'mean': hr_mean, 'std': hr_STD, 'maxTime': maxtime, 'minTime': mintime,
                    'meanNN': meanNN, 'sdNN': sdnn, 'sdaNN1': sdnna1, 'sdNNI1': sdnni1, 'sdaNN2': sdnna2,
                    'sdNNI2': sdnni2, 'sdaNN5': sdnna5, 'sdNNI5': sdnni5,
                    'medianNN': medianBB, 'madNN': madBB, 'hcvNN': hcvnn, 'iqrNN': iqrange, 'pNN50': pnn50,
                    'pNN20': pnn20})
        if (len(bbinan) > 1):
            out.update({
                'sdsd': time_domain_features['sdsd'], 'rmssd': time_domain_features['rmssd'],
                'cvNN': time_domain_features['cvnni'], 'cvsd': time_domain_features['cvsd'],
                'tiNN': geometrical_features['tinn'], 'HTI': geometrical_features['triangular_index'],
                'sd1': poincare_features['sd1'], 'sd2': poincare_features['sd2'],
                'sd1sd2': poincare_features['ratio_sd2_sd1'],
    ...
    def preprocess(self) -> np.array:
        ...
        filled_hrv = self.inputation(grid_hrv)
    ...
    @staticmethod
    def inputation(grid_hr: dict) -> dict:
