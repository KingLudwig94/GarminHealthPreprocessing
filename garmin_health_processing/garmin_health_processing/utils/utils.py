...
def retime(data: dict, step: int,  maxTime: int, startTime: int = 0, cumulative: bool = False) -> dict:
    """ function to retime data on uniform grid

    Args:
        data (dict): input data
        step (int): step of grid
        startTime (int): start time of grid
    ...
    """
    grid = {}
    for i in np.arange(startTime, maxTime, step):
        grid[i] = np.nan
    count = dict.fromkeys(np.arange(startTime, maxTime, step), np.nan)
...
def showpreprocess(grid: dict, filled: dict, avged: dict, title: str, start: int = 0, save: bool = False):
    ...
    filled_times = [starttime.__add__(
        timedelta(seconds=int(t[0]))) for t in list(fill)]
    ...
    plt.plot(timings,
             np.fromiter(avged.values(), dtype=float), 'r', label="Denoised data")
    plt.title(title)
    plt.xticks(ticks=ticks, labels=labels)
    plt.legend()
    plt.show()
